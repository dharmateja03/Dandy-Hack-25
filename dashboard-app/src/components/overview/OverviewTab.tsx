...
export function OverviewTab() {
  ...
  const loadData = async () => {
    try {
      // Only load digest for now, other endpoints return 404
      const digestRes = await apiService.getTodayDigest().catch(() => ({ data: null }))
      setDigest(digestRes.data)
      
      // Try to load blockers and help requests, but don't fail if they don't exist
      apiService.getBlockers()
        .then(res => setBlockers(Array.isArray(res.data) ? res.data : []))
        .catch(() => setBlockers([]))
      
      apiService.getHelpRequests()
        .then(res => setHelpRequests(Array.isArray(res.data) ? res.data : []))
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">{digest?.summary?.total_standups || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Tasks completed</span>
                  <span className="font-semibold">{digest?.summary?.tasks_completed || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Active blockers</span>
                  <span className="font-semibold text-red-600">{digest?.summary?.blockers_detected || 0}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Help requests</span>
                  <span className="font-semibold text-yellow-600">{digest?.summary?.help_requests || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">{digest?.key_metrics?.completion_rate || 0}%</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Team Mood</span>
                  <span className="text-2xl">{digest?.key_metrics?.team_mood || '😐'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Velocity</span>
                  <span className="font-semibold">{digest?.key_metrics?.team_velocity || 0}/week</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant={digest?.key_metrics?.productivity === 'high' ? 'success' : digest?.key_metrics?.productivity === 'moderate' ? 'warning' : 'neutral'}>
                    {digest?.key_metrics?.productivity || 'low'}
                  </Badge>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">In Progress</span>
                  <span className="font-semibold">{digest?.summary?.tasks_in_progress || 0}</span>
                </div>
              </div>
            </div>
